...
import { useAllClients, useAllProjects, useCreateManagerClientTask, useUpdateManagerClientTask } from '@/hooks/useManagerPortalData'
...
/** Diálogo de criar OU editar uma tarefa avulsa em `client_tasks`
 * (aparece direto no Portal Cliente, /tasks) — mesmo molde do
 * `KanbanTaskFormDialog.tsx`, só que gravando na tabela do cliente em
 * vez do Kanban interno. Tarefa criada aqui nasce sempre "única" (sem
 * recorrência) — recorrência é um conceito de modelo (Workflows do
 * Cliente), não de tarefa avulsa. */
export function ClientTaskFormDialog({ trigger, task, defaultClientId }: ClientTaskFormDialogProps) {
  const [open, setOpen] = useState(false)
  const { data: clients } = useAllClients()
  const { data: projects } = useAllProjects()
  const createTask = useCreateManagerClientTask()
  const updateTask = useUpdateManagerClientTask()
  const isEdit = !!task
    ...
    if (next) {
      form.reset(
        task
          ...
          : { ...EMPTY_VALUES, clientId: defaultClientId ?? '' },
      )
    }
  }
  ...
  async function onSubmit(values: TaskFormValues) {
    ...
    try {
      if (task) {
        ...
        toast.success('Tarefa criada — já aparece no Portal Cliente.')
      }
      setOpen(false)
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isEdit ? 'Editar tarefa do cliente' : 'Nova tarefa do cliente'}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Ex: Enviar fotos novas dos produtos" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Ex: Criativo, Aprovação, Acesso" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
