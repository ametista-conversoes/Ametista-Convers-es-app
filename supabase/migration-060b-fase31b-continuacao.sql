-- Ametista Conversões — Fase 31b (continuação): a migration-060 já
-- converteu a coluna `activity_checklist_items.platform_scope` pra
-- array com sucesso numa tentativa anterior (confirmado: os 32 itens já
-- existentes estão como {"meta","google"}, o valor certo pra "comum").
-- Esse arquivo só faz o que ainda faltava — NÃO mexe mais no tipo da
-- coluna, pra não repetir o erro 22P02.
--
-- Como usar: copie todo este arquivo e cole no SQL Editor do painel do
-- Supabase (SQL Editor > New query), depois clique em "Run".

-- =========================================================
-- 1. Garante o default e a constraint da coluna (idempotente — se já
--    existir, só substitui pelo mesmo valor/reconfere).
-- =========================================================
alter table public.activity_checklist_items
  alter column platform_scope set default '{meta,google}'::text[];

alter table public.activity_checklist_items
  drop constraint if exists activity_checklist_items_platform_scope_check;
...
--    array — idempotente (se já converteu, o valor extraído não bate
--    com 'meta'/'google' e cai no "else", que é o mesmo array).
-- =========================================================
...
--    array em vez de um valor único (create or replace, sem risco de
--    repetir).
-- =========================================================
...
-- =========================================================
-- 4. Trava no banco (create or replace, sem risco de repetir).
-- =========================================================
...
drop trigger if exists trg_check_activity_item_platform_match on public.activity_checklist_items;
