...
## 6. Reset de senha / convite — tela `/reset-password` quebrando com "Algo deu errado" (achado ao vivo, 10/09, ainda sem causa confirmada)
- [ ] **Bug reportado pelo usuário**: tanto um link de convite/recuperação normal quanto um expirado, ao serem abertos, caem em `ametistaconversoes.app/reset-password` mas mostram a tela genérica do `ErrorBoundary` ("Algo deu errado. Recarregue a página.") em vez do formulário ou do aviso de link expirado — acontecia antes disso funcionar (ver a checagem de link expirado feita no commit `aa453a9`, que aparentemente não é a causa: a lógica dela roda antes de qualquer render de `ResetPassword.tsx`, não lança exceção).
- [ ] **Não reproduzido nem confirmado por leitura de código** — revisei `ResetPassword.tsx`, `AuthContext.tsx` e `AuthLayout.tsx` a fundo e nenhum deles tem nada que devesse lançar erro de render nas duas situações descritas. **Antes de eu tentar mais uma correção às cegas, preciso de um destes dois dados**:
  1. Configurações → Erros (só admin/gestor) — o próprio `ErrorBoundary` já loga automaticamente a mensagem e o stack exatos desse erro em `error_logs` (Fase 21.1) assim que ele acontece; a entrada mais recente com `source = frontend` é literalmente a causa raiz.
  2. **Hipótese mais provável, por analogia com o item 7 (Google Ads MCC deu `redirect_uri_mismatch` depois da troca de domínio da Fase 25)**: Supabase Studio → Authentication → URL Configuration → "Redirect URLs" pode ainda estar listando só o domínio antigo da Vercel, sem `https://ametistaconversoes.app/reset-password` (ou um wildcard `https://ametistaconversoes.app/**`) — se o link de convite/recuperação for gerado com um `redirectTo` fora da allow-list, o comportamento do Supabase nesse caso não é o mesmo que "link expirado" (que já é tratado), pode estar caindo num formato de erro diferente que a tela ainda não sabe tratar. Confirmar se essa URL está cadastrada lá é rápido e não depende de mexer em código.
- [ ] Depois de identificada a causa real (por um dos dois caminhos acima), volta a ser: pedir "Esqueci minha senha", abrir o link recebido por e-mail, confirmar que cai em `/reset-password` (não em `/login`) e consegue trocar a senha de ponta a ponta.
...
## 21. Aprovações externas do Google/Meta — bloqueiam validação com dados reais de terceiros
