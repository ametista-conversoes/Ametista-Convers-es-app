...
- Aba "Grupos de Anúncios": abaixo da lista de grupos, confirma 4 blocos novos — "Dispositivo" (custo/cliques/conversões por Celular/Computador/Tablet), "Desempenho geográfico (cidade/região)", "Top termos de pesquisa" e "Top palavras-chave" (top 10 cada, por cliques).
- Numa campanha que não seja de Pesquisa (Display/Vídeo/PMax) → "Top termos de pesquisa" e "Top palavras-chave" devem aparecer vazios com a mensagem explicando o motivo, não um erro.
- Projeto vinculado a uma campanha do Meta Ads → confirma que só o badge de tipo de campanha (objetivo) e CPC/valor de conversão aparecem; a aba "Grupos de Anúncios" (incluindo os 4 blocos novos) mostra o aviso de "só Google Ads por enquanto", sem quebrar.

## 15. Aprovações externas do Google/Meta — bloqueiam validação com dados reais de terceiros
