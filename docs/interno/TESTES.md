...
- [ ] Rodar `migration-072-plataforma-palavras-chave-projeto.sql` antes de testar.
- [ ] **Limite de caracteres não é uma constraint no banco** — só validação no formulário de criação do Catálogo, pra não quebrar nenhuma entrada antiga (algumas já passam de 15/90 caracteres, criadas antes desse limite existir).
- [ ] Catálogo → Criativos → Adicionar, Tipo "Headline" → digitar mais de 15 caracteres → contador fica vermelho ("X/15 caracteres") e o botão Salvar desabilita; apagar até 15 ou menos → volta a habilitar.
- [ ] Mesmo teste com Tipo "Descrição", limite 90 caracteres. Tipo "Frase de destaque" e "Vídeo" → sem contador, sem limite.
- [ ] Modo "Colar várias em massa" com Tipo "Headline" → colar um bloco onde uma das linhas passa de 15 caracteres → aviso mostra quantas linhas passaram do limite e o botão "Criar N entradas" desabilita até todas as linhas caberem.
- [ ] Novo projeto (Central de Informações → Projetos → Novo projeto) → campo "Plataforma" (Google Ads/Meta Ads, pré-selecionado Google Ads) → criar → confirma que aparece um badge com a plataforma escolhida na Visão Geral do projeto e na lista de Projetos da Central de Informações.
- [ ] Projeto criado antes dessa fase (sem plataforma salva) → não mostra badge nenhum (em vez de mostrar "null" ou quebrar); abrir a aba Campanha, o Select "Plataforma" aparece com "Google Ads" pré-selecionado (fallback) — salvar já grava a plataforma de verdade dali em diante.
...
- [ ] Sem migration nova, sem deploy de Edge Function — só frontend.
- [ ] **Causa raiz**: desde a Fase 30 (separação `tasks`/`client_tasks`), a página `/client-tasks` continuou lendo `public.tasks` (Kanban) filtrado por cliente — mas segundo o próprio usuário, o propósito dessa página sempre foi o gestor ver o que o CLIENTE cria/completa em `/tasks` do Portal Cliente e o que "Workflows do Cliente" aplica, ou seja, `client_tasks`. Confirmado ao vivo pelo usuário: aplicar/mandar uma tarefa que aparece certo no Portal Cliente não aparecia em `/client-tasks` do Portal Gestor. Corrigido: `/client-tasks` agora lê `client_tasks` (`useAllClientTasks`, `ManagerClientTaskRow.tsx`, `ClientTaskFormDialog.tsx`) — igual pro Kanban (`/kanban`, `tasks`), que **não muda em nada**.
- [ ] Testar: Workflows → "Workflows do Cliente" → aplicar um modelo a um cliente (ou o próprio cliente criar uma tarefa avulsa em `/tasks`) → confirma que a tarefa aparece em `/client-tasks` do Portal Gestor na hora (sem precisar mexer em nada do Kanban).
- [ ] Trocar o status de uma tarefa em `/client-tasks` (Portal Gestor) → confirma que reflete em `/tasks` do Portal Cliente e vice-versa (mesma linha, 2 telas) — usa a mesma RPC `set_client_task_status` dos dois lados.
- [ ] "Nova tarefa" em `/client-tasks` → cria direto em `client_tasks` (aparece no Portal Cliente), diferente do "Nova tarefa" do Kanban (`/kanban`, continua criando em `tasks`, só a agência vê). Editar (lápis) e apagar (modo de exclusão) funcionando igual ao Kanban.
...
## 30. Aprovações externas do Google/Meta — bloqueiam validação com dados reais de terceiros
