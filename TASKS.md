...
- [x] Cassie IA (Fase 7.1) — chat no portal do cliente (`/cassie`, antes só um placeholder), via API da OpenAI (o usuário já tinha essa chave, decidiu não pagar duas assinaturas de IA diferentes). Modelo `gpt-5-mini`. Nova Edge Function `cassie` (rota `/chat`) autentica o cliente, monta um resumo dos dados reais dele (plano, health score, projetos com cpa/roas/ctr/spend/revenue, tarefas em aberto/atrasadas, métricas dos últimos 30 dias) e manda isso + o histórico da conversa (`cassie_messages`, nova tabela) pra API da OpenAI (Responses API — confirmado nome do endpoint/modelo/preço direto na documentação oficial deles, já que a API mudou desde o que eu tinha em memória). Só no portal do cliente, nada mudou no portal do gestor. Testado ao vivo: página carrega sem erro (fica em "Carregando..." até a migração rodar, esperado)
- [ ] Pendente: rodar `migration-027-fase7-cassie.sql`, colar o código de `supabase/functions/cassie/index.ts` como Edge Function nova no painel do Supabase, e adicionar o segredo `OPENAI_API_KEY` (gerado em platform.openai.com, colado direto no painel — nunca no código) — sem isso a Cassie fica presa em "Carregando..." e não responde nada ainda
