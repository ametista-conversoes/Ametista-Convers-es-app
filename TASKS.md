...
- [x] **Testes automatizados (Fase 7.3)**:
  ...
  - Playwright configurado (`playwright.config.ts`, sobe o `npm run dev` sozinho e reaproveita se já tiver rodando) — `npm run test:e2e`. 3 testes: login→navegação→logout (**passa**), aplicar workflow cria as tarefas certas (**passa**, corrigido depois de descobrir ao vivo que criar Workflow Operacional é ação exclusiva de admin — gestor só lê e aplica), e pedir revisão gera a tarefa (chega até o fim da interação do cliente, mas a tarefa não aparece pro gestor ainda — falta rodar a `migration-030`)
  - [ ] Pendente: rodar `migration-030-fase73-tarefa-revisao.sql` — depois disso os 3 testes Playwright devem passar
