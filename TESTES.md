...
## 3. "Failed to fetch" no celular — provável causa raiz encontrada (CORS), aguardando confirmação do domínio
- **Atualização (07/09)**: você confirmou que o erro acontece toda vez, imediatamente ao abrir a tela, em wifi normal, sem VPN/DNS privado/bloqueador — ou seja, não é bloqueio de tela nem rede instável.
- **Achado**: as 3 Edge Functions envolvidas (`cassie`, `integrations`, `client-access`) têm `Access-Control-Allow-Origin` fixo em `'https://ametistaconversoes.app'` (hardcoded). O próprio `PROJETO.md` (linha 79) já alertava sobre isso: "se o domínio final for diferente de `https://ametistaconversoesapp.vercel.app`, atualizar corsHeaders... senão Cassie e integrações param por CORS". Se o site em produção não estiver servido exatamente nesse domínio (`ametistaconversoes.app`, sem "www", https), o navegador bloqueia a resposta da Edge Function por CORS — e isso é reproduzível 100% das vezes, bate exatamente com o que você reportou. As chamadas que continuam funcionando na mesma tela (nome do cliente, contas vinculadas) são Postgrest via supabase-js, que não passa por essa checagem de origem.
- **Falta eu confirmar com você**: qual é a URL exata que aparece na barra de endereço quando você usa o app (produção) — no celular e no computador. Preciso saber se é `ametistaconversoes.app`, `www.ametistaconversoes.app`, `ametistaconversoesapp.vercel.app`, `ametistaconversoes.com`, ou outra — daí eu troco o `corsHeaders` das 3 funções pra aceitar o(s) domínio(s) reais (com um allowlist, não mais um valor fixo, pra não repetir esse problema se o domínio mudar de novo).
